...
void loop() {
  ...
  while (millis() - previousMillis < 1000) {         // https://arduino.stackexchange.com/a/12591
    int pinDoorStateRead1 = analogRead(pinCurrent);  // 112 us, 8900 times per second
    ...
    if (pinDoorStateRead1 > 10) powerOnLastSecondTimes += 1;
  }
  ...
  if (powerOnPrevious == 2) {  // initialize state
    ...
    if (powerOnPrevious == 0 && powerOnCurrent == 1) {
      ...
      if (powerOffSeconds > 2) {  // debounced
